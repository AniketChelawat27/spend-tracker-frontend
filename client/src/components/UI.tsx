        ...
        <thead>
          <tr className="border-b border-border dark:border-border-dark bg-neutral-50 dark:bg-neutral-800/80">
            {columns.map((col) => (
              <th
                key={col.key}
                className={`text-left text-xs font-semibold uppercase tracking-wider text-neutral-500 dark:text-neutral-400 px-3 sm:px-5 py-3 sm:py-4 ${col.className ?? ''}`}
              >
                {col.label}
              </th>
            ))}
...
export const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`${base} ${variants[variant]} ${sizes[size]} touch-manipulation ${disabled ? 'opacity-50 cursor-not-allowed' : ''} ${className}`}
    >
      {children}
    </button>
  );
...
export const Input: React.FC<InputProps> = ({
  ...
  <div className={`flex flex-col gap-2 ${className}`}>
    ...
    <input
      ...
      min={min}
      step={step}
      className="rounded-xl border border-border dark:border-border-dark bg-white dark:bg-neutral-800 px-4 py-2.5 min-h-[44px] text-neutral-900 dark:text-neutral-100 placeholder:text-neutral-400 focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500 transition-all touch-manipulation"
    />
  </div>
);
...
export const Select: React.FC<SelectProps> = ({
  ...
  <div className={`flex flex-col gap-2 ${className}`}>
    ...
    <select
      ...
      onChange={onChange}
      required={required}
      className="rounded-xl border border-border dark:border-border-dark bg-white dark:bg-neutral-800 pl-4 pr-10 py-2.5 min-h-[44px] text-neutral-900 dark:text-neutral-100 focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500 cursor-pointer transition-all appearance-none bg-[length:18px] bg-[right_12px_center] bg-no-repeat touch-manipulation"
...
export const TextArea: React.FC<TextAreaProps> = ({
  ...
  <div className={`flex flex-col gap-2 ${className}`}>
    ...
    <textarea
      ...
      placeholder={placeholder}
      rows={rows}
      className="rounded-xl border border-border dark:border-border-dark bg-white dark:bg-neutral-800 px-4 py-2.5 min-h-[88px] text-neutral-900 dark:text-neutral-100 placeholder:text-neutral-400 focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500 resize-none transition-all touch-manipulation"
    />
  </div>
);
...
export const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children }) => {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-neutral-900/50 backdrop-blur-sm animate-fade-in"
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-t-2xl sm:rounded-3xl border border-border dark:border-border-dark shadow-float-lg w-full max-w-md max-h-[92vh] sm:max-h-[90vh] overflow-hidden flex flex-col animate-slide-up">
        <div className="sticky top-0 flex items-center justify-between px-4 sm:px-6 py-4 sm:py-5 border-b border-border dark:border-border-dark bg-white dark:bg-neutral-900 z-10 shrink-0">
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-2.5 rounded-xl text-neutral-500 hover:bg-neutral-100 dark:hover:bg-neutral-800 hover:text-neutral-700 dark:hover:text-neutral-200 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center touch-manipulation"
            aria-label="Close"
          >
            ×
          </button>
